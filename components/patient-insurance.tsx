...
import { Shield, CreditCard, CheckCircle2, FileText, ArrowRight, DollarSign } from "lucide-react";
...
export function PatientInsurance({ patients, appointments }: PatientInsuranceProps) {
  ...
  const claimsHistory = useMemo(() => {
    ...
    const services = [
      { code: "99213", desc: "Outpatient Visit", charges: 120, cover: 100, patientPay: 20 },
      { code: "85025", desc: "Complete Blood Count", charges: 45, cover: 45, patientPay: 0 },
      { code: "99214", desc: "Clinical Care Visit", charges: 180, cover: 150, patientPay: 30 },
      { code: "71046", desc: "Chest X-Ray", charges: 90, cover: 72, patientPay: 18 },
    ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-50">Insurance Coverage</h2>
          <p className="text-sm text-muted-foreground">Manage insurance policies, view copays, and track claims</p>
        </div>
        <select
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_1fr]">
        ...
        <div className="flex flex-col justify-between overflow-hidden rounded-2xl bg-gradient-to-br from-[#10b981] via-[#059669] to-[#047857] p-6 text-white shadow-xl min-h-[220px]">
          <div className="flex items-start justify-between">
            <div className="space-y-1">
              <span className="text-[10px] font-bold uppercase tracking-wider opacity-80">Health Plan Card</span>
              <h3 className="text-2xl font-extrabold tracking-tight">{insuranceInfo.primaryInsurance}</h3>
            </div>
          ...
          <div className="mt-8 grid grid-cols-2 gap-4">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-wider opacity-70">Member Name</p>
              <p className="text-sm font-bold tracking-wide">{selectedPatient.name}</p>
            </div>
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-wider opacity-70">Member ID</p>
              <p className="font-mono text-sm font-bold">{policyId}</p>
            </div>
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-wider opacity-70">Group Number</p>
              <p className="font-mono text-sm font-bold">{groupId}</p>
            </div>
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-wider opacity-70">Copay (PCP/Spec)</p>
              <p className="text-sm font-bold">{insuranceInfo.primaryCopay} / {patientIndex % 2 === 0 ? "$45" : "$35"}</p>
            </div>
          </div>
        ...
        <Card className="rounded-xl border border-slate-100 bg-white shadow-sm dark:border-slate-800 dark:bg-card">
          <CardHeader className="pb-3">
            <CardTitle className="text-md font-bold">Policy Information</CardTitle>
            <CardDescription>Primary coverage details and verify active benefits</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-y-3 text-sm">
              <span className="text-muted-foreground">Carrier:</span>
              <span className="font-semibold text-right">{insuranceInfo.primaryInsurance}</span>
              
              <span className="text-muted-foreground">Secondary Carrier:</span>
              <span className="font-semibold text-right">{insuranceInfo.secondaryInsurance}</span>
              
              <span className="text-muted-foreground">Primary Copay:</span>
              <span className="font-semibold text-right text-teal-600 dark:text-teal-400">{insuranceInfo.primaryCopay}</span>
              
              <span className="text-muted-foreground">Secondary Copay:</span>
              <span className="font-semibold text-right text-teal-600 dark:text-teal-400">{insuranceInfo.secondaryCopay}</span>
              
              <span className="text-muted-foreground">Dental Plan:</span>
              <span className="font-semibold text-right text-emerald-600 dark:text-emerald-400">Covered (100% preventive)</span>
              
              <span className="text-muted-foreground">Vision Plan:</span>
              <span className="font-semibold text-right text-emerald-600 dark:text-emerald-400">Covered ($10 Copay)</span>
            </div>
            ...
            <div className="border-t border-slate-100 pt-4 dark:border-slate-800">
              <Button 
                ...
                disabled={verifying}
                className="w-full bg-[#3d3bdc] text-white hover:bg-[#3230b7]"
              >
                {verifying ? "Verifying Benefits..." : verifiedSuccess ? "Benefits Active!" : "Verify Eligibility"}
              </Button>
            </div>
          </CardContent>
        </Card>
      ...
      <Card className="rounded-xl border border-slate-100 bg-white shadow-sm dark:border-slate-800 dark:bg-card">
        <CardHeader>
          <CardTitle className="text-md font-bold">Recent Claims & Submissions</CardTitle>
          <CardDescription>Statements filed with the insurer for medical services</CardDescription>
        </CardHeader>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {claimsHistory.map((claim) => (
                  <tr key={claim.id} className="hover:bg-slate-50/30 dark:hover:bg-slate-900/10">
                    ...
                    <td className="px-6 py-4">
                      <div>
                        ...
                        <p className="text-[10px] text-muted-foreground">Code: {claim.code}</p>
                      </div>
                    </td>
                    <td className="px-6 py-4 font-medium">${claim.billed.toFixed(2)}</td>
